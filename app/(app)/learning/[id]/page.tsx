...
export default function ModuleDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {canEdit && module.resources.some((r) => r.type === "Worksheet") && (
        <ShareWorksheetLink moduleId={module.id} />
      )}
...
function ShareWorksheetLink({ moduleId }: { moduleId: string }) {
  ...
  async function copy() {
    try {
      ...
      toast("Couldn't copy — select and copy the link", "error");
    }
  }
  ...
  return (
    <Card className="mt-4 border-accent/30 bg-accent/5 p-4">
      <div className="flex items-start gap-3">
        <Icons.Share2 className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
        <div className="min-w-0 flex-1">
